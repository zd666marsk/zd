...
def _import_first_available(*candidates):
    ...
    raise ModuleNotFoundError(
        "无法定位以下任一模块: {}".format(", ".join(candidates))
    ) from last_error


Material = None
CarrierListFromG4P = None
Vector = None
signal_convolution = None
output = None
_relative_import_error = None

if __package__:
    try:
        from .model import Material as _Material  # type: ignore
        from .interaction.carrier_list import CarrierListFromG4P as _CarrierListFromG4P  # type: ignore
        from .util.math import Vector as _Vector, signal_convolution as _signal_convolution  # type: ignore
        from .util.output import output as _output  # type: ignore
    except ImportError as exc:  # pragma: no cover - 仅在包外执行时触发
        _relative_import_error = exc
    else:
        Material = _Material
        CarrierListFromG4P = _CarrierListFromG4P
        Vector = _Vector
        signal_convolution = _signal_convolution
        output = _output

if Material is None:
    try:
        Material = _import_first_available(
            "model",
            "raser.model",
            "current.model",
        ).Material
        ...
        if _relative_import_error is not None:
            raise _relative_import_error from exc
        raise
...
class FieldCache:
    ...
    def __init__(self, my_f, resolution=5.0):
        ...
        logger.info(f"电场缓存初始化完成，分辨率: {resolution} um")
    ...
    def get_e_field_cached(self, x, y, z):
        try:
            ...
            logger.warning(f"电场缓存获取失败 ({x:.1f}, {y:.1f}, {z:.1f}): {exc}")
            return self._safe_get_e_field(x, y, z)
    ...
    def get_doping_cached(self, x, y, z):
        try:
            ...
            logger.warning(f"掺杂浓度获取失败 ({x:.1f}, {y:.1f}, {z:.1f}): {exc}")
            return 0.0
    ...
    def _safe_get_e_field(self, x, y, z):
        try:
            ...
            logger.error(f"电场获取失败，使用默认值: {exc}")
    ...
    def _safe_get_doping(self, x, y, z):
        try:
            ...
            logger.warning(f"掺杂浓度获取失败，使用默认值: {exc}")
            return 0.0
...
class VectorizedCarrierSystem:
    ...
    def __init__(self, all_positions, all_charges, all_times, material, carrier_type="electron",
        ...
        logger.info(
            f"探测器尺寸: {self.detector_params['l_x']:.1f} × {self.detector_params['l_y']:.1f} × "
            f"{self.detector_params['l_z']:.1f} um"
        )
    ...
    def _create_material_safe(self, material):
        try:
            ...
            logger.warning(f"Material对象创建失败 {material}，使用默认硅材料: {exc}")
            try:
                return Material("si")
            except Exception:
    ...
    def _get_param_safe(self, my_d, param_name, default, param_type=float):
        try:
            ...
            logger.warning(f"参数 {param_name} 转换失败，使用默认值 {default}: {exc}")
            return default
    ...
    def _calculate_correct_mobility(self, temperature, doping, charge, electric_field):
        try:
            ...
            logger.warning(f"迁移率计算失败，使用默认值: {exc}")
    ...
    def _check_boundary_conditions(self, x, y, z):
        ...
        return (
            x <= -tolerance or x >= l_x + tolerance or
    ...
    def _get_e_field_safe(self, field_cache, x, y, z, idx):
        try:
            ...
            logger.warning(f"载流子 {idx} 电场获取失败: {exc}")
            self.active[idx] = False
    ...
    def _calculate_diffusion(self, diffusion_constant, mu):
        try:
            diffusion = diffusion_constant * math.sqrt(max(mu, 0.0))
            return (
    ...
    def _log_progress(self, step, total_carriers):
        ...
        progress = (total_carriers - active_count) / total_carriers * 100.0
    ...
    def _log_final_stats(self, start_time, max_steps):
        ...
        logger.info(
            f"批量漂移完成: 共{self.performance_stats['total_steps']}步，耗时{total_time:.2f}秒"
        )
        logger.info(
            f"最终状态: {final_stats['active_carriers']}个活跃，平均步数{final_stats['average_steps']:.1f}"
        )
    ...
    def update_original_carriers(self, original_carriers):
        ...
        for i, carrier in enumerate(original_carriers):
            ...
            try:
                ...
                logger.warning(f"更新载流子 {i} 时出错: {exc}")
