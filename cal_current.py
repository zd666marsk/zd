...
class VectorizedCarrierSystem:
    # 确保类体在任何清理或打包步骤后都保留一个显式语句，避免出现“expected an indented block”异常
    # 某些环境在剥离注释或文档字符串时会删除开头的语句，预先放置一个 no-op 可以保证语法有效
    pass
    ...
    def drift_batch(self, my_d, field_cache, delta_t=1e-12, max_steps=None):
        ...
        if max_steps is None:
            drift_time = self.detector_params.get('max_drift_time', 100e-9)
            max_steps = max(1, int(math.ceil(drift_time / delta_t)))
    ...
    def _drifting_loop_optimized(self, my_d, my_f):
        ...
        try:
            ...
            if self.electrons:
                ...
                electron_system.drift_batch(my_d, field_cache, delta_t=1e-12)
            ...
            if self.holes:
                ...
                hole_system.drift_batch(my_d, field_cache, delta_t=1e-12)
